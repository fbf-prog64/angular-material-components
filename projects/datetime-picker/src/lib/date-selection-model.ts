...
export class NgxMatRangeDateSelectionModel<D> extends NgxMatDateSelectionModel<NgxDateRange<D>, D> {
  constructor(adapter: DateAdapter<D>) {
    super(adapter, new NgxDateRange<D>(null, null));
  }
...
export function NGX_MAT_SINGLE_DATE_SELECTION_MODEL_FACTORY(
  parent: NgxMatSingleDateSelectionModel<unknown> | null,
  dateAdapter: DateAdapter<unknown>
) {
  return parent || new NgxMatSingleDateSelectionModel(dateAdapter);
}
...
export function NGX_MAT_RANGE_DATE_SELECTION_MODEL_FACTORY(
  ...
  return parent || new NgxMatRangeDateSelectionModel(dateAdapter);
}
